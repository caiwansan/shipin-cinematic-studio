<template>
  <!--
    HealthPage.page-shell.vue — 将 HealthPage.vue 迁移到 PageShell

    与原始文件的差异：
    1. 页面结构由 PageShell 编排，移除了手动 layout（.max-w-6xl.mx-auto.px-4.py-6）
    2. PageHeader（title/description/breadcrumbs）由 PageShell 管理
    3. Summary slot → Brand Health 总分和关键指标（ADI Overall Score）
    4. Content slot → 品牌选择器 + 维度卡片 + 页面主要内容
    5. Explain slot → Explain 说明区域
    6. Next slot → 下一步操作
    7. 状态管理（loading/error/empty）委托给 PageShell 的 displayState
    8. 导航链接以 breadcrumbs 形式交给 PageShell

    迁移后验证四种状态：
    - loading: 展示 LoadingState skeleton
    - error: 展示 ErrorState 卡片
    - empty: 展示 EmptyState（无品牌项目 / 无评估数据）
    - default: 正常展示所有 slots
  -->
  <PageShell
    title="品牌健康"
    description="查看品牌在 AI 眼中的健康状态"
    :loading="loading && !hasProjects"
    :error="errorObj"
    :empty="emptyConfig"
    :breadcrumbs="breadcrumbs"
  >
    <!-- Summary: Brand Health 总分和关键指标 -->
    <template #summary>
      <!-- ===== Section 1: ADI Overall Score (Primary KPI) ===== -->
      <div v-if="hasReportData" class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div class="flex items-start justify-between flex-wrap gap-4">
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <h1 class="text-2xl font-bold text-gray-900">评估</h1>
              <span class="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full font-medium">ADI 分数</span>
            </div>
            ...
            <p class="text-xs text-gray-400 mt-0.5">
              上次更新：{{ formatDate(store.lastUpdated) }}
            </p>
          </div>
    ...
    <template #content>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-4 mb-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <button class="geo-btn geo-btn--secondary" @click="refreshAll" :disabled="refreshing">
            {{ refreshing ? '刷新中…' : '刷新' }}
          </button>
        </div>
      </div>
      ...
      <GeoCard v-if="error" title="⚠️ 加载错误" className="health-error-card">
        <p class="text-sm">{{ error }}</p>
        <button class="geo-btn geo-btn--primary mt-2" @click="refreshAll">重试</button>
      </GeoCard>
      ...
      <template v-if="hasReportData">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          <h2 class="text-lg font-semibold text-gray-900 mb-4">ADI 维度</h2>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-gray-900">ADI 趋势</h2>
            <div class="flex items-center gap-2">
              <button
          ...
          <div v-else class="text-center py-8 text-sm text-gray-400">
            {{ timelineData.length === 0 ? '暂无趋势数据' : '图表需要至少 2 个数据点' }}
          </div>
        </div>
      </template>
    </template>

    <!-- Explain: Explain 说明区域 -->
    <template #explain>
      <!-- 当前页面在状态管理和说明方面由内联控制，此处保留 explain slot 以供未来扩展 -->
    </template>
...
const emptyConfig = computed<EmptyStateConfig | null>(() => {
  ...
  if (hasProjects.value && noData.value && !loading.value) {
    return {
      type: 'no-results',
      title: '暂无评估数据',
      description: '该品牌尚未扫描，请在品牌详情页开始扫描',
      actionLabel: '开始扫描',
      onAction: goToDetail,
    }
  }
  if (!hasProjects.value && !loading.value) {
    return {
      type: 'new-user',
      title: '暂无品牌项目',
      description: '在品牌扫描中创建项目以开始评估',
      actionLabel: '创建品牌',
