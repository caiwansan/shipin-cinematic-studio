<template>
  <div class="verification-section" v-if="report.verification">
    <div class="flex items-center gap-2 mb-4">
      ...
      <h3 class="text-base font-semibold text-gray-900">验证 — 验证结果</h3>
    </div>

    <!-- Before / After Comparison -->
    <div class="flex items-center justify-center gap-6 mb-5 p-5 bg-gray-50 rounded-xl">
      <div class="text-center">
        <div class="text-xs text-gray-500 uppercase mb-1">优化前</div>
      ...
      <div class="text-center">
        <div class="text-xs text-gray-500 uppercase mb-1">优化后</div>
        <div class="text-3xl font-extrabold text-emerald-600">{{ report.verification.afterAdi }}</div>
      </div>
      <div class="border-l border-gray-200 pl-6">
        <div class="text-xs text-gray-500 uppercase mb-1">提升幅度</div>
    ...
    <div v-if="report.verification.breakdown.length > 0" class="mb-5">
      <h4 class="text-sm font-semibold text-gray-700 mb-3">提升明细</h4>
      <div class="space-y-1.5">
        <div class="flex items-center gap-3 px-3 py-2 bg-gray-50 rounded-lg text-sm">
          <span class="text-gray-500 font-medium flex-1">基线</span>
    ...
    <div v-if="report.verification.remainingIssues.length > 0">
      <h4 class="text-sm font-semibold text-gray-700 mb-3">未解决项</h4>
      <div class="space-y-1.5">
        <div
        ...
        >
          ...
          <span class="text-sm text-gray-500">差距：{{ issue.gap }}</span>
          <span
  ...
  <div v-else class="verification-section--empty">
    <div class="flex items-center gap-2 mb-2">
      <span class="text-lg">✅</span>
      <h3 class="text-base font-semibold text-gray-900">验证</h3>
    </div>
    <p class="text-sm text-gray-400">暂无验证数据。完成验证步骤后可查看结果。</p>
  </div>
</template>
