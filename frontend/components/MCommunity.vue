  ...
  <div class="mcomm">
    ...
    <div v-if="activeTab === 'community' || activeTab === 'friends'" class="mcomm-feed">
      ...
      <div v-for="p in posts" :key="p.id" class="mcomm-post" @click="openDetail(p)">
        ...
        <div class="mcomm-content" :class="{ clamp: !p._expanded }">{{ cleanPostText(p.content) }}</div>
    ...
    <div v-if="postDetailOn" class="mcomm-mask">
      <div class="mcomm-modal mcomm-detail">
        ...
        <div class="mcomm-detail-body">{{ cleanPostText(postDetailOn.content) }}</div>
...
async function publish() {
  const content = cleanAiText(text.value.trim())
...
function useAi() { if (aiResult.value) { text.value = cleanAiText(aiResult.value).slice(0, maxLen.value); aiOpen.value = false; mobileToast('✅ 已填入发帖框') } }

// 清洗 AI/Markdown 书写符号（* # - _ ` ~ 」…）：只留正文，去掉装饰性符号
function cleanAiText(s: string): string {
  ...
  let t = String(s)
    ...
    .replace(/^#{1,6}[ \t]+/gm, '')                // 标题 #
    .replace(/^[>\s]+/gm, '')                       // 引用 >
    .replace(/^[\s]*[-*+][ \t]+/gm, '• ')          // 无序列表项 → 圆点
    .replace(/^[\s]*\d+[.、)][ \t]+/gm, '')        // 有序列表编号
    .replace(/\*\*([^*\n]+)\*\*/g, '$1')            // 粗体 **x**
    .replace(/__([^_\n]+)__/g, '$1')                  // 下划线
    .replace(/\*([^*\n]+)\*/g, '$1')                  // 斜体 *x*
    .replace(/^_([^_\n]+)_$/gm, '$1')                 // 整行斜体 _x_
    ...
    .replace(/^[\s]*([-*_=]){3,}[\s]*$/gm, '\n')   // 分隔线 ---/***/___
  // 清理行首/行尾的多余符号与空白
  t = t.replace(/^[\s]*[*_~#`-]{1,2}[ \t]/gm, '')
      .replace(/[ \t]+$/gm, '')
      .replace(/\n{3,}/g, '\n\n')
      .trim()
  return t
}
function cleanPostText(v: any) { return typeof v === 'string' ? cleanAiText(v) : v }
