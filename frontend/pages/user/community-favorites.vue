      ...
      <div v-else class="post-list">
        <div v-for="p in items" :key="p.id" class="post-item">
          <NuxtLink :to="`/community/post/${p.id}`" class="post-main">
            ...
            <div v-if="p.summary" class="post-summary">{{ p.summary }}</div>
          ...
          <button class="del-btn" title="取消收藏" @click="unfavorite(p.id)">🔖</button>
        </div>
      </div>
...
async function load(page = 1) {
  loading.value = true
  try {
    const res = await $fetch(`/api/community/my/favorites?page=${page}&pageSize=20`, {
      headers: { Authorization: `Bearer ${getAuthToken()}` },
    })
    items.value = res.favorites || []
...
async function unfavorite(postId: string) {
  try {
    ...
    if (res && (res as any).favorited === false) {
      items.value = items.value.filter(x => x.id !== postId)
    }
  } catch (e: any) {
...
.post-summary { font-size: 0.82rem; color: #8a8373; margin-bottom: 8px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
...
.del-btn {
  ...
  opacity: 0.55; transition: opacity 0.2s; padding: 4px;
}
