<template>
  <div class="kh-page">
    ...
    <div class="kh-header">
      <h1>实体图谱</h1>
      <p class="kh-subtitle">实体关系管理与知识信号</p>
    </div>

    <!-- Loading -->
    <div v-if="loading" class="kh-loading-grid">
      <div v-for="n in 4" :key="n" class="kh-skeleton-card"><div class="skeleton-block" style="height:1.2rem;width:50%"></div><div class="skeleton-block" style="height:0.85rem;width:30%;margin-top:10px"></div><div class="skeleton-block" style="height:0.85rem;width:100%;margin-top:10px"></div></div>
    </div>
    ...
    <!-- Empty -->
    <div v-else-if="entities.length === 0" class="kh-empty">
      <div class="kh-empty-icon">🔗</div>
      <h3>暂无实体数据</h3>
      <p>实体关系会在创建品牌和产品后自动构建。</p>
      <p class="kh-hint">涵盖：品牌、公司、产品、创始人、技术、行业、认证、合作伙伴、客户、竞品、地点</p>
    </div>

    <!-- Data -->
    <div v-else class="kh-content-list">
      <div v-for="entity in entities" :key="entity.id" class="kh-card">
        <div class="kh-card-header">
          <span class="kh-card-icon">🔗</span>
          <div class="kh-card-title-group">
            <h3 class="kh-card-title">{{ entity.name }}</h3>
            <span class="kh-card-badge">{{ entity.type }}</span>
          </div>
        </div>
        <p class="kh-card-desc">{{ entity.description }}</p>
        <div v-if="entity.aliases && entity.aliases.length" class="kh-aliases">
          <span class="kh-label">别名</span>
          <div class="kh-alias-list">
            <span v-for="(alias, i) in entity.aliases" :key="i" class="kh-tag">{{ alias }}</span>
          </div>
        </div>
        <div v-if="entity.relations && entity.relations.length" class="kh-relations">
          <span class="kh-label">关联关系 ({{ entity.relations.length }})</span>
          <div class="kh-relation-list">
            <span v-for="(rel, i) in entity.relations.slice(0, 4)" :key="i" class="kh-rel-item">
              {{ rel.relationType }} → {{ rel.targetEntityName }}
            </span>
            <span v-if="entity.relations.length > 4" class="kh-rel-more">+{{ entity.relations.length - 4 }}</span>
          </div>
        </div>
      </div>
    </div>
...
import type { Entity } from '~/modules/knowledge-hub/types/index';
...
onMounted(async () => {
  try {
    entities.value = await knowledgeApi.getEntities();
  } catch (e) {
    console.error('KH Entity load error:', e);
    error.value = true;
  } finally {
    loading.value = false;
  }
...
.kh-card-badge { display: inline-block; font-size: 0.7rem; color: #7c3aed; background: #f5f3ff; padding: 2px 8px; border-radius: 4px; margin-top: 4px; }
.kh-card-desc { font-size: 0.82rem; color: #6b7280; line-height: 1.5; margin: 0 0 12px; }
.kh-aliases { margin-bottom: 10px; }
.kh-label { font-size: 0.75rem; font-weight: 600; color: #374151; display: block; margin-bottom: 6px; }
.kh-alias-list { display: flex; flex-wrap: wrap; gap: 4px; }
.kh-tag { font-size: 0.72rem; color: #374151; background: #f3f4f6; padding: 2px 8px; border-radius: 4px; }
.kh-relations { margin-bottom: 4px; }
.kh-relation-list { display: flex; flex-wrap: wrap; gap: 4px; }
.kh-rel-item { font-size: 0.72rem; color: #065f46; background: #d1fae5; padding: 2px 8px; border-radius: 4px; }
.kh-rel-more { font-size: 0.72rem; color: #9ca3af; padding: 2px 8px; }
</style>
