      ...
      <section class="dl-hero">
        ...
        <div class="dl-grid">
          <div v-for="p in products" :key="p.id" class="dl-card" :style="{ borderTop: '3px solid ' + p.accent }">
            ...
            <div class="dl-zip" v-if="p.meta && p.meta.zipUrl">
              <a :href="p.meta.zipUrl" class="dl-btn dl-btn-zip">
                <span class="dl-btn-icon">📦</span>
                便携版下载（zip 免安装{{ p.meta.zipSize ? ' · ' + p.meta.zipSize : '' }}）
              </a>
            </div>
...
function toCard(raw: any): ProductCard {
  ...
  return {
    ...
    meta: raw.version ? {
      ...
      zipUrl: raw.zipUrl ?? '',
      zipSize: raw.zipSize ? ((raw.zipSize / 1048576).toFixed(1)) + ' MB' : '',
      zipSha256: raw.zipSha256 ?? '',
    } : null,
  }
}
...
.dl-btn-zip {
  ...
  padding: 10px 18px;
  border-radius: 10px;
  background: linear-gradient(120deg, rgba(34,211,238,.15), rgba(129,140,248,.12));
...
.dl-btn-zip:hover { background: linear-gradient(120deg, rgba(34,211,238,.28), rgba(129,140,248,.22)); box-shadow: 0 4px 16px rgba(34,211,238,.25); }
</style>
