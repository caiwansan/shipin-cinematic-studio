<template>
  <div>
    <div class="mb-6">
      <h1 class="text-lg font-semibold text-white">QQ扫码登录配置</h1>
      <p class="text-xs text-gray-500 mt-1">配置 QQ 互联的扫码登录，用户可在登录页使用 QQ 扫码登录</p>
    </div>
    ...
    <div class="bg-[#0D1328] rounded-xl border border-[#1A2240] p-6 max-w-2xl">
      <div class="space-y-5">
        <div class="form-group">
          ...
          <input v-model="form.appId" type="text" placeholder="QQ互联 AppID" class="form-input" />
        </div>

        <div class="form-group">
          <label class="block text-xs text-gray-400 mb-1.5">AppSecret</label>
          <div class="flex gap-2">
            <input v-model="appSecretDisplay" type="password" placeholder="QQ互联 AppSecret" class="form-input flex-1" />
        ...
        <div class="form-group">
          ...
          <input v-model="form.redirectUri" type="text" placeholder="https://aigc.fushtn.com/api/auth/qq/callback" class="form-input" />
          <p class="text-[10px] text-gray-600 mt-1">在 QQ 互联平台配置的回调地址，默认 https://aigc.fushtn.com/api/auth/qq/callback</p>
        </div>

        <div class="form-group flex items-center gap-3">
          <label class="text-xs text-gray-400">启用 QQ 登录</label>
          <label class="toggle">
    ...
    <div class="bg-[#0D1328] rounded-xl border border-[#1A2240] p-6 max-w-2xl mt-6">
      ...
      <ol class="space-y-2 text-xs text-gray-400 leading-relaxed">
        <li>1. 前往 <a href="https://connect.qq.com/" target="_blank" class="text-blue-400 underline">QQ互联</a> 注册成为开发者</li>
        <li>2. 创建"网站应用"获取 AppID 和 AppSecret</li>
        <li>3. 在 QQ互联 配置授权回调域为：<code class="text-amber-400 bg-[#1a1a28] px-1.5 py-0.5 rounded">aigc.fushtn.com</code></li>
...
const form = ref({
  appId: '',
  appSecret: '',
  redirectUri: 'https://aigc.fushtn.com/api/auth/qq/callback',
  enabled: true,
})
...
async function loadConfig() {
  try {
    const res = await fetch('/api/admin/qq-oauth/config', {
    ...
    if (data.data?.config) {
      ...
      form.value.redirectUri = data.data.config.redirectUri || 'https://aigc.fushtn.com/api/auth/qq/callback'
      form.value.enabled = data.data.enabled !== false
    } else if (data.config) {
      form.value.appId = data.config.appId || ''
      form.value.appSecret = data.config.appSecret || ''
      form.value.redirectUri = data.config.redirectUri || 'https://aigc.fushtn.com/api/auth/qq/callback'
...
async function saveConfig() {
  ...
  try {
    ...
    form.value.appSecret = appSecretDisplay.value
...
.form-input:focus { border-color: rgba(81,149,255,0.4); }
.btn-primary {
  background: linear-gradient(135deg, #5195ff, #3b82f6);
  border: none;
  color: white;
  cursor: pointer;
...
.toggle input:checked + .toggle-slider { background: #5195ff; }
