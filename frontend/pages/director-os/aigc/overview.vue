<template>
  <div class="space-y-6">
    ...
    <template v-else>
      <!-- ⭐ 仪表盘 -->
      <div>
        <div class="text-[10px] text-gray-500 uppercase mb-3">📊 数据概览</div>
        <div class="space-y-3">
          <!-- 行1: 新增会员 -->
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">今日新增会员</div>
              <div class="text-lg font-bold text-white">{{ dashboard.todayNewUsers }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">当月新增会员</div>
              <div class="text-lg font-bold text-white">{{ dashboard.monthNewUsers }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">今日新增 VIP</div>
              <div class="text-lg font-bold text-yellow-400">{{ dashboard.todayNewVip }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">当月新增 VIP</div>
              <div class="text-lg font-bold text-yellow-400">{{ dashboard.monthNewVip }}</div>
            </div>
          </div>
          <!-- 行2: 收益+代理+佣金+待提现 -->
          <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">今日收益</div>
              <div class="text-lg font-bold text-green-400">¥{{ formatMoney(dashboard.todayRevenue) }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">当月收益</div>
              <div class="text-lg font-bold text-green-400">¥{{ formatMoney(dashboard.monthRevenue) }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">当月新增代理</div>
              <div class="text-lg font-bold text-blue-400">{{ dashboard.monthNewAgents }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">新增佣金</div>
              <div class="text-lg font-bold text-purple-400">¥{{ formatMoney(dashboard.newCommission) }}</div>
            </div>
            <div class="bg-[#0D1328]/80 border border-[#1A2240] rounded-xl p-3.5">
              <div class="text-[10px] text-gray-500 uppercase mb-1">待提现金额</div>
              <div class="text-lg font-bold text-orange-400">¥{{ formatMoney(dashboard.pendingWithdraw) }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8">
        <div class="flex items-center justify-between mb-3">
          ...
          <NuxtLink to="/director-os/aigc/models" class="text-[10px] text-blue-400 hover:text-blue-300 no-underline">查看全部 →</NuxtLink>
        </div>
...
const quickLinks = [
  { label: 'VIP 套餐', icon: '👑', to: '/director-os/aigc/vip', desc: '会员套餐配置' },
  { label: 'VIP 订单', icon: '📋', to: '/director-os/aigc/vip-orders', desc: 'VIP付款订单审核' },
  { label: '大模型列表', icon: '🤖', to: '/director-os/aigc/models', desc: '管理 API Key' },
  { label: '会员模块', icon: '👥', to: '/director-os/aigc/members', desc: '用户列表管理' },
  { label: '支付设置', icon: '💳', to: '/director-os/aigc/payment', desc: '微信/支付宝' },
  { label: '管理员设置', icon: '🛡️', to: '/director-os/aigc/admins', desc: '权限管理' },
]
...
async function fetchData() {
  ...
  try {
    ...
    if (token) {
      ...
      if (res.ok) {
        const d = await res.json()
        if (d.success && d.data) dashboard.value = d.data
      }
    }
  } catch {}
