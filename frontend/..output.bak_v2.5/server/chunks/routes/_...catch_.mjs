...
const KNOWN_CLIENT_PATHS = /* @__PURE__ */ new Set([
  "/",
  "/login",
  "/mobile",
  "/projects",
...
const STATIC_ASSETS = /^\/(_nuxt|favicon|logo|robots\.txt|sitemap)/;
const API_PATH = /^\/api\//;
const ____catch_ = defineEventHandler((event) => {
  const path = event.path || "/";
  if (API_PATH.test(path)) return;
  if (STATIC_ASSETS.test(path)) return;
  if (KNOWN_CLIENT_PATHS.has(path)) return;
...
export { ____catch_ as default };
//# sourceMappingURL=_...catch_.mjs.map
