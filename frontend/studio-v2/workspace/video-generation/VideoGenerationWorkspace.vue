<template>
  <div class="video-workspace-rework">
        ...
        <div v-else class="segment-detail">
          ...
          <div class="collapse-section open">
            ...
            <div class="detail-header" style="margin-top:12px;">
              ...
              <button class="btn-generate-video" :disabled="generatingVideo || !hasStoryboardFrames" @click="generateCurrentVideo" :title="!hasStoryboardFrames ? '请先生成分镜图' : ''">
                {{ generatingVideo ? '⏳ 生成中…' : (!hasStoryboardFrames ? '🎬 需分镜图' : '🎬 生成视频') }}
              </button>
            </div>
            </div>
            ...
            <div v-if="generatedVideos[selectedSegmentIndex]" class="generated-video-section" style="margin-top:12px;">
              ...
              <div class="gen-feedback-actions" style="display:flex;gap:8px;margin-top:8px;justify-content:center;">
                <button class="sb-feedback-btn sb-fb-like" @click="recordVideoAction('like')" title="有用" style="background:transparent;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px 12px;font-size:13px;cursor:pointer;">👍</button>
                <button class="sb-feedback-btn sb-fb-dislike" @click="recordVideoAction('dislike')" title="不满意" style="background:transparent;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px 12px;font-size:13px;cursor:pointer;">👎</button>
                <button class="sb-feedback-btn sb-fb-fav" @click="recordVideoAction('favorite')" title="收藏" style="background:transparent;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px 12px;font-size:13px;cursor:pointer;">⭐</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="framePickerShow" class="picker-overlay" @click.self="framePickerShow=false">
      <div class="picker-dialog">
        <div class="picker-header">
          <span class="picker-title">选择{{ {firstFrame:'首帧图',lastFrame:'尾帧图'}[framePickerMode] }}</span>
...
const hasStoryboardFrames = computed(() => {
  const ws = (state as any)?.workspace
  // storyboardImages 是分镜图生成后填充的数组（优先使用）
  const images = ws?.storyboardImages || []
  if (Array.isArray(images) && images.length > 0) return true
  // fallback: 旧版 storyboard.frames 结构
  const frames = ws?.storyboard?.frames || ws?.frames || []
...
// 🧪 R1: 记录视频行为信号（纯采集）
async function recordVideoAction(action: string) {
  const seg = segments.value?.[selectedSegmentIndex.value]
  const videoUrl = generatedVideos.value[selectedSegmentIndex.value]
  if (!videoUrl) return
  try {
    await fetch('/api/evaluation/record-action', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        assetId: videoUrl.split('/').pop() || videoUrl,
        assetType: 'video',
        userAction: action,
...
async function generateCurrentVideo() {
  ...
  // ⭐ 分镜图是视频生成的前置必要条件
  if (!hasStoryboardFrames) {
    alert('⚠️ 请先生成分镜图，视频生成需要分镜图作为空间参考。')
  ...
  try {
    ...
    let firstUrl = segmentFirstFrameUrls.value[idx] || ''
    const midUrl = segmentMidFrameUrls.value[idx] || ''
    const lastUrl = segmentLastFrameUrls.value[idx] || ''

    // ⭐ 自动 fallback：如果首帧图为空但 storyboardImages 存在，从分镜图取
    if (!firstUrl) {
      const sbImages: any[] = (state as any)?.workspace?.storyboardImages || []
      if (sbImages.length > 0) {
        // 按 segmentId 匹配当前段
        const segId = seg.id || seg.segmentId || String(idx)
        const match = sbImages.find(s => s.segmentId === segId || s.segmentId === 'seg_' + String(idx).padStart(3, '0'))
        // 或用 int 索引匹配
        const fallback = match || sbImages[idx] || sbImages[0]
        if (fallback?.imageUrl) {
          firstUrl = fallback.imageUrl
          console.log('[generateCurrentVideo] ⭐ 自动取分镜图作为首帧参考:', firstUrl?.substring(0, 60))
        }
      }
    }''
