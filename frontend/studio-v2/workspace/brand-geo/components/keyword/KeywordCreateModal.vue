    ...
    <div class="geo-modal">
      ...
      <div class="geo-modal-body">
        <div class="geo-form-group">
          ...
          <textarea v-model="keywordsText" class="geo-input geo-textarea" placeholder="输入关键词，每行一个" rows="6"></textarea>
        </div>
        <div class="geo-form-group">
          <label class="geo-form-label">类型</label>
          <select v-model="keywordsType" class="geo-input">
      ...
      <div class="geo-modal-footer">
        ...
        <button class="geo-btn geo-btn-primary" @click="$emit('save', { keywordsText: keywordsText, keywordsType: keywordsType })" :disabled="saving || !keywordsText.trim()">
          {{ saving ? '添加中...' : '添加' }}
        </button>
      </div>
    </div>
...
const emit = defineEmits<{ close: []; save: [data: { keywordsText: string; keywordsType: string }] }>()

const keywordsText = ref('')
const keywordsType = ref('brand')
</script>
