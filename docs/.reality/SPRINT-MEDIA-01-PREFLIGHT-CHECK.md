...
## 0.2 掌柜最终验收 Gate（权威版，2026-08-02 01:23 确认）
> 第一阶段 Reality Gate 最终验收标准（取代实施计划 §7 旧版 E1-E6 表述，以本版为准）：
...
E5 数据同步回昆仑镜
  ↓
E6 Reality Report 完成
```

第一条真实内容发布成功 = 新媒体工作台从「代码存在」进入「产品存在」的分界线。
验收不看代码量：账号连接成功 → AI员工运营入口 → 真实发布一篇 → 微信后台可见 → 数据回流昆仑镜。

## 0.3 掌柜确认的冻结范围（Phase 1 不做）
❌ 自动选题 ❌ 自动生成运营计划 ❌ 私信机器人 ❌ 企业微信 ❌ 微信群运营 ❌ 客户价值评分 ❌ 多平台矩阵
→ 全部属于后续 AI Employee 能力层 Sprint。
