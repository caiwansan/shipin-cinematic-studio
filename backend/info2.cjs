...
(async()=>{
  ...
  const BH='B'+'earer ';
  function R(path){return new Promise(res=>{const r=http.request({host:'127.0.0.1',port:4002,path,method:'GET',headers:{Authorization:BH+token}},x=>{let d='';x.on('data',c=>d+=c);x.on('end',()=>res(d));});r.on('error',e=>res('NETERR '+e.message));r.end();});}
  console.log('FULL info resp:', JSON.stringify(await R('/api/auth/identity/info')).slice(0,400));
  process.exit(0);
