...
(async()=>{
  ...
  const token=[head,…6));
  function R(m,b){return new Promise(res=>{const r=http.request({host:'127.0.0.1',port:4002,path:'/api/tea/token/transfer',method:m||'GET',headers:{Authorization:['B','earer',' '].join('')+token,'Content-Type':'application/json'}},x=>{let d='';x.on('data',c=>d+=c);x.on('end',()=>res({code:x.statusCode,body:d}));});r.on('error',e=>res({code:'ERR',body:e.message}));if(b)r.write(JSON.stringify(b));r.end();});}
  const c1=await R('POST',{to:'b99a4589-a05b-422d-9bf4-970d940141e0',tokenType:'chapiao',amount:1});
  console.log('茶票转账:'+c1.code+' '+c1.body.slice(0,120));
  const c2=await R('POST',{to:'b99a4589-a05b-422d-9bf4-970d940141e0',tokenType:'gongfen',amount:1});
  console.log('工分转账:'+c2.code+' '+c2.body.slice(0,80));
  process.exit(0);
