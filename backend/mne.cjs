...
(async()=>{
  ...
  const show=async(lb,p,m,b)=>{const r=await R(p,m,b);console.log('['+lb+'] -> '+r.code+' '+String(r.body).slice(0,200).replace(/\n/g,' '));};
  await show('status','/api/tea/storage/status');
  await show('mnemonic-gen','/api/tea/storage/mnemonic','POST');
  await show('status-after','/api/tea/storage/status');
  process.exit(0);
