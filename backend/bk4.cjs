...
(async()=>{
  ...
  const dep=await R('/api/tea/bank/deposit','POST',{amount:20,days:30});
  console.log('deposit ->', JSON.stringify(dep));
  const st=await R('/api/tea/bank/status');
  const ds=(st.data&&st.data.deposits)||[];
  console.log('deposits count:', ds.length);
  if(ds.length){ const did=ds[ds.length-1].id; const wd=await R('/api/tea/bank/deposit/withdraw','POST',{depositId:did}); console.log('withdraw ->', JSON.stringify(wd)); }
  process.exit(0);
