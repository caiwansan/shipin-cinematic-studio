...
export default async function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/auth/refresh', async (request, reply) => {
    ...
    try {
      ...
      // 同 IP 刷新不递增版本（多端共存）；跨 IP 刷新视为新会话（互踢）
      const dbUser = await prisma.user.findUnique({ where: { id: decoded.id }, select: { tokenVersion: true, activeIp: true } }).catch(() => null)
      const newTokenVersion = await nextTokenVersion(
        { id: decoded.id, tokenVersion: dbUser?.tokenVersion ?? decoded.tokenVersion ?? 1, activeIp: dbUser?.activeIp ?? null },
        clientIp
      )
