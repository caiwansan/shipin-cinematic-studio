...
export function calculateGapLevel(entry: CoverageEntry): GapLevel | undefined {
  if (!entry.gap) return undefined
  if (entry.primaryDatasets.length === 0 && entry.secondaryDatasets.length === 0) return 'P0'
  if (entry.primaryDatasets.length === 0) return 'P1'
  return 'P2'
}
