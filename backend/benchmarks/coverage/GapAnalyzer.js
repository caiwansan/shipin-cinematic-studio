...
const CoverageIndex_js_1 = require("./CoverageIndex.js");
...
function analyzeGaps(entries) {
    ...
    for (const entry of entries) {
        if (!entry.gap)
            continue;
        const level = (0, CoverageIndex_js_1.calculateGapLevel)(entry);
        if (!level)
            continue;
        // 获取能力定义和组信息
        const def = registry_js_1.CapabilityRegistry.byId(entry.capability);
        const groupId = def?.group ?? 'UNKNOWN';
        const suggestedId = `${entry.difficulty}-${groupId}`;
        // 获取同组的其它能力作为建议的 secondary
        const groupCapabilities = registry_js_1.CapabilityRegistry.query({ group: groupId })
            .map(c => c.id)
        ...
        if (level === 'P0')
            critical.push(suggestion);
        else if (level === 'P1')
            weak.push(suggestion);
        else if (level === 'P2')
            sparse.push(suggestion);
    }
