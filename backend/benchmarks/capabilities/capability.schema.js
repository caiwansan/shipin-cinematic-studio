 ...
 * 在注册时自动调用。
 */
function validateCapability(cap, knownIds) {
    const errors = [];
    // ID 格式：全大写 + 下划线
    ...
    if (!cap.name)
        errors.push(`Capability "${cap.id}": name is required`);
    if (!cap.description)
        errors.push(`Capability "${cap.id}": description is required`);
    return errors;
}
/**
 * 验证整个 Registry 的完整性。
...
function validateRegistry(registry) {
    ...
    if (ids.size !== registry.all.length) {
        ...
        for (const [id, count] of seen) {
            if (count > 1)
                errors.push(`Duplicate capability id: "${id}" (${count}x)`);
        }
    }
    // 逐条验证
    for (const cap of registry.all) {
    ...
    for (const stage of allStages) {
        ...
        if (count === 0)
            warnings.push(`Stage "${stage}" has zero capabilities`);
    }
