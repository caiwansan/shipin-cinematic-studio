...
const byGroup = new Map<CapabilityDefinition['group'], CapabilityDefinition[]>()
const byStage = new Map<CapabilityDefinition['stage'], CapabilityDefinition[]>()
const byDifficulty = new Map<CapabilityDefinition['difficulty'], CapabilityDefinition[]>()
...
function buildIndexes() {
  for (const cap of CAPABILITIES) {
    ...
    if (!byStage.has(cap.stage)) byStage.set(cap.stage, [])
    byStage.get(cap.stage)!.push(cap)

    if (!byDifficulty.has(cap.difficulty)) byDifficulty.set(cap.difficulty, [])
