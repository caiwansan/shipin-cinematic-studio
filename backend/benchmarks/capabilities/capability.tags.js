...
exports.recommendForScenario = recommendForScenario;
const TAGS = {
    CAMERA_PATH: {
        id: 'CAMERA_PATH',
        label: '摄像机路径',
...
function searchByKeyword(keyword) {
    ...
    return Object.values(TAGS).filter(t => t.label.includes(kw) ||
        t.keywords.some(k => k.includes(kw)));
}
function recommendForScenario(scenario) {
    const s = scenario.toLowerCase();
    return Object.values(TAGS).filter(t => t.recommendedScenario.some(rs => rs.includes(s)));
}
