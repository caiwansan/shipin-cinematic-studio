...
export const aiRouter = {
  ...
  scoreModel(model: any, policy: any): number {
    ...
    const successRate = totalExecutions > 0
      ...
      : 0.95 // 初始未使用模型给 95% 置信度

    // 延迟评分：越低越好，阈值设定为该类模型典型值
    const latencyThreshold = model.modelType === 'video' ? 120
      : model.modelType === 'image' ? 20
      : model.modelType === 'tts' ? 5
      : 10
    ...
    const loadPenalty = loadRatio > 0.8 ? (loadRatio - 0.8) * 0.5 : 0
