...
export type VideoFormat = 'wan2.7-i2v' | 'wan2.6-legacy' | 'wan2.7-r2v' | 't2v-standard'
    ...
    switch (format) {
      case 'wan2.7-r2v': {
        ...
        if (input.r2vMedia?.length) {
          body.input.media = input.r2vMedia.map(m => ({
            type: m.type,
            url: m.url,
      ...
      case 'wan2.7-i2v': {
        // 新版图生视频: input.media[{type:"first_frame"/"last_frame", url}]
      ...
      case 'wan2.6-legacy': {
        ...
        if (input.imageUrl2) body.input.img_url2 = input.imageUrl2
