...
export default fp(async function authPlugin(fastify: FastifyInstance) {
  ...
  fastify.addHook('onRequest', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const tier = dbUser.memberTier || 'free'
    // 只有 VIP 会员需要检查私有 key
    if (tier === 'free' || tier === 'basic') return

    // VIP 会员必须有至少一个私有 API Key
    console.warn("[LEGACY-V1-READ] plugins/auth.ts — VIP Key 检查读 V1 UserModelConfig, 应迁移到 V2");
    const [hasApiKey, hasModelConfig] = await Promise.all([
      prisma.userApiKey.findFirst({ where: { userId, isActive: true } }),
      prisma.userModelConfig.findFirst({ where: { userId, apiKey: { not: null } } }),
    ])
    if (!hasApiKey && !hasModelConfig) {
