...
/**
 * Try to load user's model config for a given capability
 */
async function tryLoadUserModelConfig(
  ...
  try {
    console.warn("[LEGACY-V1-READ] model-selection/index.ts — 读 V1 findFirst");
    ...
    // Check if the specific capability is enabled
    const enabledField = fields.enabledColumn
    const isEnabled = config[enabledField]
    if (!isEnabled) return null

    // Get the specific model and provider for this capability
    const modelName = config[fields.modelColumn]
    const providerColumn = config[fields.providerColumn]
    const provider = providerColumn || config.provider
