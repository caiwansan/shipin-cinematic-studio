...
import { checkGraphIntegrity } from '../closure/execution-graph-frozen.js'
...
export async function runtimeBoot() {
  ...
  // EGIL: execution graph integrity check
  console.log('[boot] EGIL integrity check...')
  const egil = checkGraphIntegrity()
  if (!egil.pass) {
    console.warn('[boot] ⚠️ EGIL: ' + egil.message)
    egil.details.forEach(d => console.warn('[boot] ⚠️ ' + d))
  } else {
    console.log('[boot] EGIL: ' + egil.message)
  }
