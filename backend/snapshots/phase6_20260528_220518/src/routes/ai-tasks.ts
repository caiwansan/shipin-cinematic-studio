...
export default async function aiTaskRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/tasks/ai-generate', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    // 验证项目归属（00000000- 前缀的自动创建型 project 自动创建）
    let project: any
    try {
    ...
    try {
      ...
      console.log(`[ai-tasks] 🎯 能力路由: 用户 ${user.id.substring(0,8)} capability=${taskTypeToCapability(taskType)} provider=${resolved.provider} model=${resolved.model}`)
    } catch (err: any) {
  ...
  // POST /api/tasks/batch-create — 批量创建 AI 任务
