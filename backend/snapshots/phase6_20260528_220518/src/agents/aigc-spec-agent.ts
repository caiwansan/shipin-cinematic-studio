...
class AigcSpecAgent {
  ...
  constructor() {
    this.systemPrompt = readFileSync(
      join(__dirname, '../prompts/aigc-spec-system.txt'),
      'utf-8'
    )
  }
  ...
  async generateSpec(options: GenerateSpecOptions): Promise<GenerateSpecResult> {
    ...
    const userPrompt = this._buildUserPrompt(text, title, aspectRatio)
...
请严格按照 JSON Schema 格式输出，只输出该类型对应的数据。`
  ...
  private _buildUserPrompt(text: string, title?: string, aspectRatio?: string): string {
    return `请根据以下故事文本，生成完整的 AIGC 制作规格表 JSON。

故事标题：${title || '未命名故事'}
用户选择的画幅比例：${aspectRatio || '16:9'}
  ...
  private async _retryWithCorrection(
    ...
    try {
      const response = await callLLM(
        [
          { role: 'system', content: this.systemPrompt + correction },
          {
            role: 'user',
            content: `故事标题：${title || '未命名故事'}\n画幅比例：${aspectRatio || '16:9'}\n\n故事文本：${text.slice(0, MAX_TEXT_LENGTH)}`,
          },
        ],
