 ...
 * AI Provider Center 首批 13 家模型供应商目录（upsert by code，可重复执行）
 * 掌柜指令 2026-08-01：大模型注册中心
 * BYOK：本表仅聚合官方入口，不保存用户 Key、不代理充值
 */
...
const providers = [
  ...
  },
  {
    ...
    officialWebsite: 'https://www.meituan.com',
    registerUrl: 'https://www.meituan.com',
    billingUrl: 'https://www.meituan.com',
    documentationUrl: 'https://www.meituan.com',
  },
  // ─────────────── 海外 ───────────────
  {
...
const byCat = await prisma.aiProviderDirectory.groupBy({ by: ['category'], _count: true })
console.log(`✅ seed 完成：共 ${total} 家`, JSON.stringify(byCat))
