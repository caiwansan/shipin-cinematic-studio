...
export const volcengineVideoAdapter: ModelAdapter = {
  ...
  async execute(runtime: RuntimePayload, input: ModelAdapterInput): Promise<ModelAdapterResult> {
    ...
    // ⭐ 物理规则（分镜和非分镜都附加——长视频后段一致性需要强物理约束）
    const physicsRules = [
      ...
      '【多角色同帧规则】当画面中有两个或以上角色时，角色之间的身高、体型比例必须在整个视频中保持完全一致。矮的角色始终比高的角色矮，不能出现角色身高在镜头切换后发生变化的错误。角色间的相对高度比例是固定的，不会随镜头角度或时间推移而改变。',
      '【物理一致性】整个视频中人物和物体的外观、形态、颜色、位置必须保持稳定不变。不能出现人物头上冒烟、物体凭空消失或变形等违反物理规律的现象。人物形态、服装、发型在视频全程保持一致。',
    ].join('\n')
    // 分镜模式和非分镜模式都附加物理规则（分镜省略禁止文字规则）
    const physicsDirective = physicsRules
