...
export type TimeoutTier = 'fast' | 'normal' | 'batch' | 'long'
...
const TIMEOUT_MAP: Record<TimeoutTier, number> = {
  ...
  long: 300_000,    // 深度分析（完整制作规格书，生成量大）
}
...
export class LLMRouter {
   ...
   *
   * ◆ 不依赖 provider.models 列表（已清空，模型名从 DB 直接传入 call({model})）
   * ◆ model 字段此时留空，调用方在执行时从上下文获取用户配置的 model
   */
  /**
   * 按 provider name 读取运行时模型名
   * ◆ 优先从 env 读（由 narrativeGateway.execute 中 buildExecutionGraph 注入）
   * ◆ 其次是 provider.models[0]
   * ◆ 最后 fallback 到空字符串（会抛错）
   */
  /**
   * 按 provider name 读取运行时模型名
   * ◆ 优先从 env 读（由 narrativeGateway.execute 中 buildExecutionGraph 注入）
   * ◆ 备选兼容 key（旧版可能是 ALIYUN 而非 BAILIAN）
   * ◆ 最后尝试 provider.models[0]
   */
  private resolveModel(providerName: string): string {
    const model = process.env[`${providerName.toUpperCase()}_LLM_MODEL`]
    if (model) return model
    // 别名兼容
    const aliasEnvVarMap: Record<string, string> = {
      bailian: 'ALIYUN_LLM_MODEL',
      aliyun: 'ALIYUN_LLM_MODEL',
    }
    const aliasVar = aliasEnvVarMap[providerName]
    if (aliasVar) {
      const aliasModel = process.env[aliasVar]
  ...
  getProviderList(override?: string[]): ProviderConfig[] {
    ...
    // ═══ 无 override 时，遍历所有有 key 的 provider ═══
    // 模型从 env / provider.models 读取
...
export class NarrativeLLMGateway {
  ...
  private async injectUserApiKey(userId: string): Promise<(() => void) | null> {
    ...
    if (!userId || userId === 'anonymous') {
      ...
      // anonymous 用户 fallback 到系统公共 V2 配置或环境变量
      const cfg = await getRuntimeConfig({
        userId: '__system_anonymous__',
        requestId: `inject-${Date.now()}`,
      })
      if (!cfg.user) {
        console.warn(`[NarrativeGateway] ❌ anonymous 用户无可用 fallback 配置`)
        return null
      }
      assertConfigIntegrity(cfg)
      return applyUserKey(cfg.user)
    }
  ...
  async execute(request: GatewayRequest): Promise<GatewayResponse> {
    ...
    // ⭐ 配额检查：所有 LLM 调用必须通过每日限额
    if (request.userId && request.userId !== 'anonymous' && !request.userId.startsWith('__system_')) {
      try {
        const { checkDailyQuota, incrementDailyUsage } = await import('../services/usage-quota.service.js')
        const quota = await checkDailyQuota(request.userId)
        if (!quota.canProceed) {
          return {
            success: false,
            content: '',
            error: `QUOTA_LIMIT_REACHED: 今日 AI 调用次数已达上限（${quota.limit} 次）`,
            tokenUsage: null,
          }
        }
        // 异步更新用量（不阻塞调用）
        incrementDailyUsage(request.userId, 'llm').catch(() => {})
      } catch { /* quota check must never break execution */ }
    }
    ...
    try {
      ...
      const provKey = final.provider.toUpperCase()
    ...
    // ===== 2. Get provider list =====
    // ⭐ 从 ExecutionGraph final 节点提取用户配置的 provider，作为唯一选择
    // 用户配什么就用什么，禁止 fallback 兜底
    const userProvider = (this as any)._lastUserProvider
    // provider 别名映射：用户配置的 provider → registry 注册名
    const providerAlias: Record<string, string> = { aliyun: 'bailian' }
    const rawProvider = executionGraph?.final?.provider || ''
    const mappedProvider = providerAlias[rawProvider] || rawProvider
    const executionProvider = mappedProvider ? [mappedProvider] : undefined
    const providerOverride = userProvider
      ? [userProvider]
      : request.providerOverride || executionProvider
...
/**
 * 将用户的 API Key 注入到 process.env（esbuild 兼容的独立函数）
 */
function applyUserKey(userCfg: { apiKey: string; model?: string; provider: string }): (() => void) {
  const backup: Record<string, string | undefined> = {}
  const { apiKey, model, provider } = userCfg

  // ⭐ 从 provider 名称映射到 envKeyPrefix（禁止硬编码映射）
  // 规则：provider id 转为大写 = 默认 prefix；特殊映射从 DB route_config 读取
  // 比如 siliconflow → SILICONFLOW, aliyun → ALIYUN, volcengine → VOLCENGINE
  ...
  console.log(`[NarrativeGateway] 🔑 V2注入: ${provider} → ${provKey}_API_KEY (model=${model})`)

  return () => {
    for (const [k, v] of Object.entries(backup)) {
      if (v !== undefined) process.env[k] = v
    }
    console.log(`[NarrativeGateway] 🔑 已恢复 Key 环境变量`)
  }
}
