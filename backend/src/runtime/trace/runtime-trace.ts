 ...
 * CommonJS: __dirname → 需要上溯 3 层从 src/runtime/trace 到项目根
 */
const REPORTS_DIR = (() => {
  return path.resolve(__dirname, '../../../../reports/runtime-trace')
})()
...
class RuntimeTraceStore {
  ...
  flush(): void {
    ...
    try {
      ...
      const filepath = pathModule.join(REPORTS_DIR, filename)
