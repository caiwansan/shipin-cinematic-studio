...
export const geoGraphService = {
  ...
  async buildGraph(projectId: string): Promise<KnowledgeGraph> {
    ...
    const graphInput = {
      entities: entities.map(mapPrismaEntity),
      relations: relations.map(mapPrismaRelation),
    }
    ...
    await prisma.gEOProjectVersion.create({
      data: {
        projectId,
        version: nextVersion,
        label: `graph-v${nextVersion}`,
        graphData: result.graph as any,
        metadata: { buildType: 'auto', builtAt: new Date().toISOString() },
      },
    })
  ...
  async getGraph(projectId: string): Promise<KnowledgeGraph | null> {
    ...
    return {
      entities: entities.map(mapPrismaEntity),
      edges: relations.map(mapPrismaRelation),
      metadata: {
        projectId,
        buildVersion: 1,
  ...
  /**
   * Get a specific node (entity) in the graph.
   */
  async getGraphNode(projectId: string, entityId: string): Promise<{ node: Entity; edges: EntityRelation[] } | null> {
    ...
    return {
      node: mapPrismaEntity(entity),
      edges: edges.map(mapPrismaRelation),
    }
  },
  ...
  async getGraphVersion(projectId: string, version: number): Promise<KnowledgeGraph | null> {
    ...
    const graphData = projectVersion.graphData as any
    if (!graphData || !graphData.entities) return null

    return graphData as KnowledgeGraph
  },

  /**
   * Return graph visualization data for D3.js force-directed layout.
   */
  async visualize(projectId: string): Promise<GraphVisualizationData | null> {
    ...
    // Assign groups based on entity type for D3 coloring
    const typeGroupMap: Record<string, number> = {
      Person: 1,
      Organization: 2,
      Concept: 3,
      Product: 4,
