...
// Engine does NOT call models directly; delegates to Step Plugins.
// ============================================================
...
export const executionEngine = {
  ...
  async execute(
    ...
    if (signal?.aborted) {
      return this._buildCancelledResult(plan, stepResults, startTime)
    }
    ...
    try {
      ...
      for (let groupIndex = 0; groupIndex < schedule.totalGroups; groupIndex++) {
        ...
        const groupPromises = groupSteps.map(scheduledStep =>
          this._executeStep(scheduledStep.step, execCtx, plan, abortController.signal),
        )
        ...
        if (failures.length > 0 && plan.rollbackPolicy?.enabled) {
          ...
          await this._rollback(plan, execCtx, abortController.signal)
          break
        }
      }
      ...
      // Build final result
      const result = this._buildResult(plan, execCtx, stepResults, startTime)
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          const timeout = step.timeout || plan.timeoutProfile?.stepTimeout || 60000
          ...
          if (output.success) {
            ...
            if (attempts < maxAttempts) {
              const backoff = this._calculateBackoff(step, attempts)
  ...
  async _rollback(
    ...
    for (const stepId of rollbackSteps.reverse()) {
      ...
      try {
        // Emit rollback events — actual cleanup is step-specific
   ...
   */
  _calculateBackoff(step: import('../types.js').ExecutionStep, attempt: number): number {
