...
export async function withUserKey<T>(
  ...
  const providerKeyMap: Record<string, string | null> = {
    volcengine: v2?.llmApiKey || v2?.imageApiKey || v2?.videoApiKey || v2?.ttsApiKey || null,
    deepseek: v2?.llmApiKey || null,
    siliconflow: v2?.llmApiKey || null,
    openai: v2?.llmApiKey || null,
    aliyun: v2?.ttsApiKey || v2?.llmApiKey || null,
    kling: null,
    replicate: null,
  }

  const encryptedKey = providerKeyMap[provider]
  if (!encryptedKey) {
    throw new Error(
  ...
  if (ctx) {
    ...
    if (v2?.baseUrl) {
      ...
      (ctx as any).secrets[baseUrlKey] = v2.baseUrl as any
    }
    (ctx as any).provider = {
      name: provider,
      apiKey: decryptedKey,
