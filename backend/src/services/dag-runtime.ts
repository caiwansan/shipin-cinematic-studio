 ...
 *
 * ⚠️ 迁移状态：旧 DAG 阶段 vs 新 PipelineDefinition
 * 旧阶段 (runtime DAG): character, scene, storyboard, voice, frame, director
 * 新阶段 (PipelineDefinition): script-analysis, specification, character-design,
 *   scene-design, props-design, storyboard, video-generation, voice-generation,
 *   music-generation, final-render
 *
 * 当前处于 OVERLAY 模式：旧 DAG 保留完整功能，同时 STAGE_LEGACY_MAP
 * 提供新旧 key 互转。未来统一后移除旧 DAG。
 */

import { prisma } from '../utils/index.js'

// ══════════════════════════════════════════════════════════
// 迁移映射：旧 DAG stage key ↔ 新 PipelineDefinition stage id
// ══════════════════════════════════════════════════════════

export const STAGE_LEGACY_MAP: Record<string, string> = {
  // 旧 (DAG) → 新 (PipelineDefinition)
...
export const STAGE_LEGACY_REVERSE: Record<string, string> = Object.fromEntries(
  Object.entries(STAGE_LEGACY_MAP).map(([k, v]) => [v, k])
)
...
/** 将 stage key 统一到新 ID 空间（如果旧 key 有映射则转换） */
export function normalizeStageKey(key: string): string {
  return STAGE_LEGACY_MAP[key] || key
}

/** 将 stage key 统一到旧 ID 空间（如果新 key 有映射则转换） */
export function denormalizeStageKey(key: string): string {
  return STAGE_LEGACY_REVERSE[key] || key
}
 ...
 * ⚠️ FROZEN — 旧 DAG 运行时
 *
 * 当前 DAG 的 stage key (character/scene/storyboard/voice/frame/director)
 * 与 PipelineDefinition (shared/pipeline-definition.ts) 不兼容。
 * 迁移完成前，此函数处理 pipeline_stages 表中旧 key 的 blocked 状态。
 *
 * 迁移策略：
 * 1. pipeline_stages 表同时创建新旧两套 stageKey 记录
 * 2. 新 stage 通过 aigc-spec/save 写入时自动同步状态
 * 3. 旧 DAG 的 blocked 计算暂保留，直到前端完全迁移到新 DAG
 */
