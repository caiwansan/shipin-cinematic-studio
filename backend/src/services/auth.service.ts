...
import { nextTokenVersion } from '../utils/session.js'
...
export const authService = {
  ...
  async login(email: string, _password: string, fastify: FastifyInstance, ip: string) {
    ...
    // 同 IP 多端共存；跨 IP 互踢（旧 IP 签发的 token 全部失效）
    const newVersion = await nextTokenVersion(user, ip)
    ...
    const token = (fastify as any).jwt.sign({ id: user.id, email: user.email, tokenVersion: newVersion, ip, organizationId } as any)
    return {
      accessToken: token,
