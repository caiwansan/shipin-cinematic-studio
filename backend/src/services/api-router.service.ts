  ...
  async selectProvider(
    ...
    if (userId && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId)) {
      ...
      const providersToTry = userSpecifiedProvider
        ? [userSpecifiedProvider, ...['volcengine', 'aliyun', 'siliconflow', 'deepseek', 'custom', 'openai'].filter(p => p !== userSpecifiedProvider)]
        : taskType === 'video' ? ['volcengine', 'aliyun', 'siliconflow', 'deepseek', 'custom', 'openai']
  ...
  async getAvailableProviders(userId?: string): Promise<{
    ...
    if (userId && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId)) {
      ...
      try {
        ...
        if (v2) {
          const all = [v2.llmProvider, v2.imageProvider, v2.videoProvider, v2.ttsProvider]
            .filter((p): p is string => !!p)
          userProviders = [...new Set(all)]
        }
      } catch { /* ignore */ }
    }
