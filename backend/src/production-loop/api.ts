...
export function registerProductionRoutes(app: FastifyInstance): void {
  ...
  // NOTE: 旧 VideoProvider 注册 (initVideoProviders) 已随 Architecture Convergence v1 删除
  // 视频/图片 Provider 统一走 ModelAdapterRegistry + RuntimeCredential
  ...
  app.post('/api/production/compile', async (req, reply) => {
    ...
    const result = await compileStoryboard(shots, context)
    return {
      shots: result.shots,
      batch: {
  ...
  // ── Video Provider Status (REMOVED: 旧 VideoProviderRegistry 已随 Architecture Convergence v1 删除) ──
  // 视频 Provider 状态请通过 ModelAdapterRegistry 查询
  app.get('/api/production/providers/video', async () => {
    return { providers: [], message: '旧 VideoProvider 已废弃，请通过 ModelAdapterRegistry 查询 Provider 状态' }
  })
