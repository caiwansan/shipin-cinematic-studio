// Render Intelligence v2 — 已废弃（Architecture Convergence v1）
...
export class RenderIntelligence {
  // ── Optimized multi-constraint decision (已废弃 — 返回固定决策) ──

  async decide(
    prompt: any,
    constraints?: any,
  ): Promise<RouteDecision> {
    const available: string[] = []  // 旧 VideoProviderRegistry 已删除
  ...
  async getIntelligenceStatus() {
    // 旧 VideoProviderRegistry 已删除，返回空状态
    return {
      available: [],
      providers: [],
      ...
      note: '旧 VideoProvider 已废弃，请通过 ModelAdapterRegistry 查询 Provider 状态',
    }
  }
  ...
  private buildDecision(
    ...
    return {
      ...
      slaTier,
      alternatives: [],  // 旧 VideoProviderRegistry 已删除
    }
  }
