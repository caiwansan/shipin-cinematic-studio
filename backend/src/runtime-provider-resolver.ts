...
export async function resolveProviderFromUserConfig(
  ...
  // 遍历所有类型，找到包含此模型的配置
  // ⭐ LLM (agent/narrative) 配置放在首位，且如果用户只配了 LLM 不配其他，仍能工作
  ...
  const isLLMTask = taskType === 'narrative' || taskType === 'llm' || taskType === 'shooting-plan'
  const typeIndex = isLLMTask ? 0 : taskType === 'frame' ? 1 : taskType === 'image' ? 1 : taskType === 'video' ? 2 : 3
  // frame 等价于 image 类型
  ...
  if (!matched) {
    ...
    if (!defaultCfg.prov || !actualKey || !defaultCfg.mdl) {
      if (isLLMTask) {
        throw new Error(`AI Agent 需要配置 LLM 的 API Key 和模型，请先在大模型设置中配置`)
      }
      throw new Error(
