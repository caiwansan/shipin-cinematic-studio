...
const OAUTH_DESKTOP_PREFIX = 'oauth:desktop:'
const oauthStateMap = new Map<string, { createdAt: number; mode?: string; ref?: string }>() // fallback if Redis down

// Desktop OAuth 结果缓存：GET callback(desktop=1) 成功后暂存 token，供壳内轮询取回

const oauthDesktopResults = new Map<string, { token: string; user: any; at: number }>() // fallback
...
// platform: 'web' = 在线版/PC浏览器, 'mobile' = 手机APP


// QQ config cache (5 min)
let _qqConfigCache: { web?: any; mobile?: any; at?: number } = {}
const QQ_CONFIG_TTL = 5 * 60 * 1000
async function getQQConfig(platform?: string) {
  try {
    if (platform === 'mobile') {
...
export default async function qqOAuthRoutes(fastify: FastifyInstance) {
  ...
  // GET /api/admin/qq-mobile/config — 管理员获取手机APP QQ配置

  fastify.get('/api/admin/qq-mobile/config', async (request, reply) => {
    ...
    const secret = await prisma.paymentSecret.findUnique({ where: { channel: 'qq_mobile' } })
  ...
  // PUT /api/admin/qq-mobile/config — 管理员保存手机APP QQ配置

  fastify.put('/api/admin/qq-mobile/config', async (request, reply) => {
    ...
    await prisma.paymentSecret.upsert({
      ...
      where: { channel: 'qq_mobile' },

      update: { config, enabled: enabled !== undefined ? !!enabled : true },

      create: { channel: 'qq_mobile', config, enabled: enabled !== undefined ? !!enabled : true, remark: 'QQ开放平台手机APP登录配置' },
  ...
  fastify.get('/api/auth/qq/authorize', async (request, reply) => {
    ...
    const config = await getQQConfig(platform)
  ...
  fastify.get('/api/auth/qq/callback', async (request, reply) => {
    ...
    if (Date.now() - stateEntry.createdAt > 10 * 60 * 1000) {
      ...
      oauthStateMap.delete(state); redis.del(OAUTH_STATE_PREFIX + state).catch(() => {})
    ...
    try {
      ...
      const config = await getQQConfig(isMobileOAuth ? 'mobile' : 'web')
      ...
      const meRes = await fetch(`https://graph.qq.com/oauth2.0/me?access_token=${accessToken}&unionid=1&fmt=json`)
      ...
      console.log("[QQ-CB-UNION] unionid=", meData ? meData.unionid : "none", " raw=", meText.slice(0,80));

      const qqUnionIdCb = meData.unionid || ''
      ...
      if (qqUnionIdCb) { const _cbrows: any = await prisma.$queryRawUnsafe(`SELECT id FROM "User" WHERE "qq_union_id"=$1 LIMIT 1`, qqUnionIdCb); if (_cbrows && _cbrows[0] && _cbrows[0].id) user = await prisma.user.findUnique({ where: { id: _cbrows[0].id } }) }

      if (!user) {

        const _rows: any = await prisma.$queryRawUnsafe(

          'SELECT id FROM "User" WHERE "qqOpenId" = $1 OR email = $2 LIMIT 1',

          qqOpenId, `qq_${qqOpenId}@aigc.fushtn.com`)

        let _uid = _rows?.[0]?.id

        if (_uid) user = await prisma.user.findUnique({ where: { id: _uid } })

        if (user && qqUnionIdCb) { await prisma.$executeRawUnsafe(`UPDATE "User" SET "qq_union_id"=$1 WHERE id=$2`, qqUnionIdCb, user.id).catch(() => {}) }
      ...
      if (user) {
        ...
        // 已绑定用户 — 正常登录（保留原 username，避免覆盖触发唯一冲突）

        await prisma.user.update({ where: { id: user.id }, data: { username: user.username, nickname: nickname || user.nickname, avatarUrl: avatarUrl || user.avatarUrl } })
        ...
        // 检查推荐人绑定状态
        const _meInv = await prisma.user.findUnique({ where: { id: user.id }, select: { inviterId: true } })
        const needBind = !_meInv?.inviterId
const JWT_SECRET = (process.env.JWT_SECRET || (() => { throw new Error("JWT_SECRET 环境变量未配置") })())
        ...
        user = await prisma.user.create({
          ...
          data: {
            ...
            username: (nickname || 'QQ用户') + '_' + email.replace('@', '').slice(-12),

            qqOpenId,

            ...(qqUnionIdCb ? { qqUnionId: qqUnionIdCb } : {}),
        ...
        if (stateEntry.ref) {
          ...
          try {
            ...
            try { await prisma.$executeRawUnsafe(`INSERT INTO "user_follow" ("id","followerId","followingId","createdAt") VALUES (gen_random_uuid(), $1::uuid, $2::uuid, now()), (gen_random_uuid(), $2::uuid, $1::uuid, now()) ON CONFLICT ("followerId","followingId") DO NOTHING`, stateEntry.ref, user.id) } catch (_eAf: any) { console.error('[QQ Invite] auto mutual follow failed:', _eAf?.message) }
  ...
  fastify.post('/api/auth/qq/callback', async (request, reply) => {
    ...
    const config = await getQQConfig((stateEntry as any)?.mobile ? 'mobile' : 'web')
    ...
    try {
      ...
      if (stateEntry.mode === 'bind') {
        ...
        // Check unionId first (cross-platform identity like PC flow)
        let mobileUser = null;
      ...
      await prisma.user.update({ where: { id: user.id }, data: { username: nickname || user.username } })
  ...
  fastify.post('/api/auth/qq/native', async (request, reply) => {
    ...
    const config = await getQQConfig('mobile')
    ...
    try {
      ...
      // 取 unionid(跨应用统一: 线上/手机同一QQ unionid 相同)

      let qqUnionId = ''

      try {

        const meRes = await fetch(`https://graph.qq.com/oauth2.0/me?access_token=${qqAccessToken}&unionid=1&fmt=json`)

        const meText = await meRes.text()

        const mm = meText.match(/unionid\":\"([^\"]+)/)

        if (mm) qqUnionId = mm[1]

      console.log("[QQ-NATIVE-UNION] unionid=", qqUnionId, " raw=", meText.slice(0,80))
      ...
      // 优先按 unionid 匹配(线上/手机同QQ归一)

      let user: any = null

      if (qqUnionId) { const _urows: any = await prisma.$queryRawUnsafe(`SELECT id FROM "User" WHERE "qq_union_id"=$1 LIMIT 1`, qqUnionId); if (_urows && _urows[0] && _urows[0].id) user = await prisma.user.findUnique({ where: { id: _urows[0].id } }) }
      ...
      if (!user) {
        ...
        if (user && qqUnionId) { await prisma.$executeRawUnsafe(`UPDATE "User" SET "qq_union_id"=$1 WHERE id=$2`, qqUnionId, user.id).catch(() => {}) }
      ...
      if (!user) {
        ...
        // 已存在用户 — 正常登录（保留原 username，避免覆盖触发唯一冲突）
      ...
      console.error("[QQ-NATIVE-ERR]", err && err.stack ? err.stack : (err && err.message ? err.message : String(err)))
  ...
  fastify.post('/api/auth/qq/attach', async (request, reply) => {
    ...
    await prisma.user.update({ where: { id: uid }, data: { qqOpenId } })
  ...
  fastify.post('/api/auth/qq/bind', async (request, reply) => {
    ...
    const existingUser = await prisma.user.findFirst({
        ...
        where: { OR: [{ phone }, { email: phone }, { email: phone + "@phone.local" }] }

      })

    if (existingUser) {

      await prisma.user.update({ where: { id: existingUser.id }, data: { qqOpenId } })
  ...
  fastify.get('/api/auth/qq/status', async (request, reply) => {

    // 平台区分：?mobile=1 返回手机APP配置状态，默认返回在线版配置状态
    ...
    return toApiResponse({enabled: !!(config && config.appId), appId: config?.appId || '', platform}) satisfies ApiResponse<unknown>;

  })



  // POST /api/auth/qq/native-bind — 原生 APP：QQ 账号绑定手机号（无需 bindToken，直接传 qqOpenId）

  // 已注册手机号 → 绑定合并（QQ 登录进该账号）；新手机号 → 以该手机号注册

  fastify.post('/api/auth/qq/native-bind', async (request: any, reply: any) => {

    const { qqOpenId, phone, code } = request.body as any

    if (!qqOpenId || !phone || !code) {
    ...
    if (bound && !String(bound.id).startsWith('x')) {

      // QQ 已绑定到某账号：若该账号恰好就是该手机号则重放(幂等)，否则拒绝

      const own = bound.phone === phone || bound.email === phone || bound.email === phone + '@phone.local'

      if (!own) return reply.status(400).send({ error: '该QQ账号已被其他用户绑定' })
    ...
    // 查找手机号对应老账号（phone / email / email@phone.local 三种）
