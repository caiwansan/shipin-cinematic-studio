...
const MODEL_DISPLAY_NAMES: Record<string, string> = {
  ...
  'doubao-seedance-2-0-pro-260510': 'Seedance 2.0 Pro',
  ...
  'doubao-seed-2-1-pro-260628': 'Seed 2.1 Pro',
  ...
  fastify.get('/api/v1/models/available', async (request, reply) => {
    try {
      ...
      const result = models.map((m) => {
        ...
        return toApiResponse({id: m.name,
          name: MODEL_DISPLAY_NAMES[m.name] || m.name,
          type: m.modelType,
          provider: m.provider,
          available,
          recommended,}) satisfies ApiResponse<unknown>;
      })

      return toApiResponse({models: result}) satisfies ApiResponse<unknown>;
    } catch (err) {
