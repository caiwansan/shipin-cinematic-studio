...
const LLM_PROVIDERS: Record<string, { base: string; models: string[] }> = {
  deepseek: { base: 'https://api.deepseek.com/v1', models: ['deepseek-v4-flash', 'deepseek-chat'] },
  volcengine: { base: 'https://ark.cn-beijing.volces.com/api/v3', models: ['doubao-seed-2-1-pro-260628', 'doubao-lite-32k'] },
  openai: { base: 'https://api.openai.com/v1', models: ['gpt-4o-mini', 'gpt-4o'] },
  aliyun: { base: 'https://dashscope.aliyuncs.com/compatible-mode/v1', models: ['qwen3.7-flash', 'qwen3.7-plus', 'qwen-max', 'qwen-plus', 'qwen-turbo'] },
...
export default async function teaLLMRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/api/tea/llm/config', auth, async (request: any) => {
  ...
  // POST /api/tea/llm/config — 保存配置
  fastify.post('/api/tea/llm/config', auth, async (request: any, reply: any) => {
    const userId = request.user.id
    const body = (request.body as any) || {}
    const cur = await getCfg(userId)
    const next = {
      provider: String(body.provider || cur?.provider || 'deepseek'),
      model: String(body.model || cur?.model || 'deepseek-v4-flash'),
      baseUrl: String((body.baseUrl ?? '') || cur?.baseUrl || ''),
  ...
  fastify.post('/api/tea/llm/chat', auth, async (request: any, reply: any) => {
    ...
    try {
      ...
      if (!r.ok) return reply.status(502).send({ success: false, error: '大模型调用失败 ' + r.status })
      const j = await r.json()
      const text = (j.choices && j.choices[0] && j.choices[0].message && j.choices[0].message.content) || ''
      return { success: true, data: { text } }
    } catch (e: any) {
      return reply.status(502).send({ success: false, error: '大模型调用失败：' + e.message })
    }
  })
