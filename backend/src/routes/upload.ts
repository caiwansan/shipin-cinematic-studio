...
export default async function uploadRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/v1/upload/asset', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
      ...
      return toApiResponse({id: asset.id,
        title: asset.title,
        type: asset.type,
        url: asset.url,
        externalAsset: true,
        message: '上传成功，此资产为外部资产，不进入交易体系'}) satisfies ApiResponse<unknown>;
    } catch (error: any) {
  ...
  fastify.post('/api/v1/upload/local', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
      ...
      if (buffer.length > MAX_SIZE) {
        return reply.status(413).send({ error: `文件过大（最大 50MB），当前 ${(buffer.length / 1024 / 1024).toFixed(1)}MB` })
      }
      ...
      return toApiResponse({id: asset.id,
        ...
        url: asset.url,
        externalAsset: true,
        message: '本地图片上传成功'}) satisfies ApiResponse<unknown>;
    } catch (error: any) {
      return reply.status(500).send({ error: `上传失败: ${error.message}` })
    }
  })
  ...
  fastify.get('/api/v1/upload/assets', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
      ...
      return toApiResponse({assets: result, total: result.length}) satisfies ApiResponse<unknown>;
    } catch (error: any) {
